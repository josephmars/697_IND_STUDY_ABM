...
to listen-threshold ;; node procedure
  ...
  let neighbor-count count link-neighbors
  let grammar-one-prop grammar-one-sum / neighbor-count

  ;; Calculate probability of adopting grammar 1 based on prestige
  let adoption-threshold threshold-val * prestige

  ifelse grammar-one-prop >= adoption-threshold
  [ set state 1 ]
...
to listen-individual
  ...
  ;; Calculate probability of adopting neighbor's grammar based on prestige
  let neighbor-state [state] of neighbor
  let adoption-prob ifelse-value (neighbor-state = 1)
    [prestige]
    [1 - prestige]
  if random-float 1.0 < adoption-prob
  [ set state neighbor-state ]
end
...
to speak ;; node procedure
  ...
  if not logistic?
  [ ;; Adjust biased-val with prestige
    let biased-val (1.5 * state * prestige)
...
to listen [heard-state] ;; node procedure
  ...
  ;; Adjust gamma based on prestige
  let adjusted-gamma gamma * prestige
  ;; choose a grammar state to be in
  ifelse random-float 1.0 <= state
  [
    ;; if grammar 1 was heard
    ifelse heard-state = 1
    [ set state state + (adjusted-gamma * (1 - state)) ]
    [ set state (1 - adjusted-gamma) * state ]
  ][
    ;; if grammar 0 was heard
    ifelse heard-state = 0
    [ set state state * (1 - adjusted-gamma) ]
    [ set state adjusted-gamma + state * (1 - adjusted-gamma) ]
  ]
end
...
@#$#@#$#@
<experiments>
  <experiment name="Prestige_Threshold_Experiment" repetitions="30" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <exitCondition>ticks &gt;= 100</exitCondition>
    <metric>(count nodes with [state = 1]) / (count nodes)</metric>
    <enumeratedValueSet variable="logistic?">
      <value value="false"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="prestige">
      <value value="0"/>
      <value value="0.2"/>
      <value value="0.4"/>
      <value value="0.6"/>
